...
function Header() {
    ...
    const { backendURL, vendorData, LogoutUser } = useAuth();
                            ...
                            <div className="absolute w-[20rem] mt-2 p-2 bg-white rounded-md shadow-lg">
                                <div className='space-y-4'>
                                    <ul>
                                        {orders.map((order) => (
                                            order.status == 'false' &&
                                            <div key={order._id} className="bg-blue-500  my-1 w-full max-w-sm p-4 border border-gray-200 rounded-lg shadow-sm dark:border-gray-700" data-v0-t="card" >
                                                <div className="flex items-start space-x-4">
                                                    <div className="flex-1 space-y-1">
                                                        ...
                                                        <div className="text-sm text-white">{order.originCity} <span className='px-3'><i className="fa-duotone fa-arrow-right fa-md text-white"></i></span> {order.departureCity}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
