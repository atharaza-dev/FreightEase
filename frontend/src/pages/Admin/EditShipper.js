...
function EditShipper() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`http://localhost:5000/api/auth/find-shippers-info/${id}`);
                if (!response.ok) {
    ...
    const updateShipper = async (e) => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/api/auth/update-shippers-info/${id}`, {
                method: 'PUT',
                headers: {
