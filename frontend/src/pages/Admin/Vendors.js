...
function Vendors() {
    ...
    useEffect(() => {
        const fetchVendorData = async () => {
            try {
                const response = await fetch('http://localhost:5000/api/auth/vendors-info');
                if (!response.ok) {
                                ...
                                <tbody class='bg-white rounded-lg'>
                                    ...
                                    {vendorData.map((vendor) => (
                                        <tr key={vendor._id} className='rounded-lg'>
                                            ...
                                            <td className="h-10 px-4 text-sm transition duration-300 border-t border-l first:border-l-0 border-slate-200 stroke-slate-500 text-slate-500">{vendor.RegNumber}</td>
